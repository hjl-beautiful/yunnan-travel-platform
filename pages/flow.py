...
with filter_col2:
    if view_mode == "年度对比":
        ...
        year_compare = st.multiselect("选择年份", available_years, 
                                       default=available_years[:min(3, len(available_years))],
                                       label_visibility="collapsed", key="year_compare")
    elif view_mode == "月度聚合":
...
with chart_col1:
    ...
    if view_mode == "年度对比" and year_compare:
        ...
        for i, yr in enumerate(year_compare):
            ...
            yr_df["day_offset"] = (yr_df["date"] - yr_df["date"].min()).dt.days
            fig.add_trace(go.Scatter(
                x=yr_df["date"], y=yr_df["visitors"],
        ...
        fig.add_trace(go.Bar(
            x=plot_df["month_name"], y=plot_df["mean"],
            name="月均客流", marker=dict(color="#3b82f6", line=dict(color="#1e3a5f", width=0.5)),
            error_y=dict(type="data", array=plot_df["std"], color="#64748b", thickness=1),
            hovertemplate="<b>%{x}</b><br>月均: %{y:,.0f} 人次<extra></extra>"
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=plot_df["month_name"], y=plot_df["max"], mode="lines+markers",
            name="月最高", line=dict(color="#f59e0b", width=1.5, dash="dot"),
            marker=dict(size=6), hovertemplate="<b>%{x}</b><br>最高: %{y:,.0f}<extra></extra>"
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=plot_df["month_name"], y=plot_df["min"], mode="lines+markers",
            name="月最低", line=dict(color="#ef4444", width=1.5, dash="dot"),
            marker=dict(size=6), hovertemplate="<b>%{x}</b><br>最低: %{y:,.0f}<extra></extra>"
        ), row=1, col=1)
    else:
        if smooth:
            ...
            fig.add_trace(go.Scatter(
                x=plot_df["date"], y=plot_df["visitors"],
                mode="lines", name="实际客流",
                line=dict(color="rgba(59,130,246,0.3)", width=1),
                hovertemplate="<b>%{x}</b><br>客流: %{y:,.0f}<extra></extra>"
            ), row=1, col=1)
            fig.add_trace(go.Scatter(
                x=plot_df["date"], y=plot_df["smoothed"],
                mode="lines", name="移动平滑",
    ...
    fig.update_layout(
        ...
        margin=dict(l=40, r=40, t=40, b=20), height=540,
        hovermode="x unified",
    )
    fig.update_xaxes(showgrid=False, zeroline=False, row=1, col=1)
    fig.update_yaxes(showgrid=True, gridcolor="rgba(100,180,255,0.06)", zeroline=False, 
                      tickformat=",", title="人次", row=1, col=1)
...
with col_a:
    ...
    fig_wd = go.Figure()
    colors_wd = ["#3b82f6"] * 5 + ["#06b6d4", "#06b6d4"]
    ...
    fig_wd.add_trace(go.Scatter(
        x=weekday_order, y=weekday_stats["mean"],
        mode="lines+markers", name="均值趋势",
...
with col_b:
    ...
    for m in range(1, 13):
        if m == peak_idx:
            colors_month.append("#f59e0b")  # 峰值金色
        elif m in [4, 5, 6, 7, 8, 9, 10, 11]:
            colors_month.append("#3b82f6")  # 旺季蓝色
        else:
            colors_month.append("#475569")  # 淡季灰色
    ...
    fig_month.update_layout(
        ...
        yaxis=dict(showgrid=True, gridcolor="rgba(100,180,255,0.06)", tickformat=",", title="月均人次"),
    )
...
with col_c:
    ...
    if len(yearly) > 1:
        ...
        fig_yr.add_trace(go.Bar(
            x=yearly["year"].astype(str), y=yearly["sum"] / 10000,
            name="年度总客流(万)", marker=dict(color="#3b82f6", line=dict(color="#1e3a5f", width=0.5)),
            text=[f"{v:.1f}万" for v in yearly["sum"] / 10000],
            textposition="outside", textfont=dict(color="#94a3b8", size=11),
            yaxis="y", hovertemplate="<b>%{x}</b><br>总客流: %{y:.1f}万<extra></extra>"
        ))
        fig_yr.add_trace(go.Scatter(
            x=yearly["year"].astype(str), y=yearly["mean"],
            name="日均客流", mode="lines+markers", yaxis="y2",
        ...
        fig_yr.update_layout(
            ...
            xaxis=dict(showgrid=False),
            yaxis=dict(showgrid=True, gridcolor="rgba(100,180,255,0.06)", title="总客流(万)", title_font_color="#64748b"),
            yaxis2=dict(showgrid=False, title="日均(人次)", title_font_color="#64748b", 
                         overlaying="y", side="right", tickformat=","),
        )
...
with col_d:
    ...
    for ins in insights:
        border_color = "#f59e0b" if ins["level"] == "warning" else ("#ef4444" if ins["level"] == "danger" else "#3b82f6")
        bg_color = "rgba(245,158,11,0.05)" if ins["level"] == "warning" else ("rgba(239,68,68,0.05)" if ins["level"] == "danger" else "rgba(59,130,246,0.05)")
        st.markdown(f"""
        <div style="background:{bg_color}; border-left:3px solid {border_color}; 
            ...
            <div style="font-size:11px; color:#94a3b8; line-height:1.5;">{ins['text']}</div>
        </div>
        """, unsafe_allow_html=True)
